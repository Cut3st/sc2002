...
public Action getPlayerAction(Player player) {
    ...
    /** Returns int[2] of item choices: 1=Potion, 2=PowerStone, 3=SmokeBomb, 4=Antidote */
    ...
    public void showBattleStatus(Combatant player, List<Combatant> enemies, int round, BattleInfo context) {
        ...
        System.out.printf("  %-20s  HP: %3d / %3d%s%n",
                player.getName(), player.getHp(), player.getMaxHp(), playerStatus);

        // Enemy rows
        for (Combatant e : enemies) {
            String status = e.isAlive() ? "" : "  [ELIMINATED]";
            String effectStatus = e.isAlive() ? formatStatus(context.getStatusSummary(e)) : "";
            System.out.printf("  %-20s  HP: %3d / %3d%s%s%n",
                    e.getName(), e.getHp(), e.getMaxHp(), status, effectStatus);
        }
